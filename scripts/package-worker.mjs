...
const source=`import app from './backend/worker.js';const files=${JSON.stringify(files)};export default {async fetch(request,env,ctx){const assets={async fetch(req){let path=new URL(req.url).pathname;if(path==='/'||!path.split('/').at(-1).includes('.'))path='/index.html';const f=files[path];if(!f)return new Response('Not found',{status:404});return new Response(req.method==='HEAD'?null:Uint8Array.from(atob(f.data),c=>c.charCodeAt(0)),{headers:{'Content-Type':f.type,'Content-Encoding':'gzip','Cache-Control':path==='/index.html'||path==='/sw.js'?'no-cache':'public, max-age=86400'}});}};return app.fetch(request,{...env,ASSETS:assets},ctx);},scheduled:app.scheduled};`;

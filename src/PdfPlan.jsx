...
export default function PdfPlan({url,page,onPages,onError,children,onClick}) {
 ...
 useEffect(()=>{let alive=true,loading;async function load(){const data=new Uint8Array(await (await privateFile(url)).arrayBuffer());if(!alive)return;loading=getDocument({data,isEvalSupported:false,useWasm:false});const doc=await loading.promise;if(alive){P(doc);onPages(doc.numPages);}}load().catch(e=>{if(alive)onError('PDF konnte nicht geladen werden: '+e.message);});return()=>{alive=false;task.current?.cancel();loading?.destroy();};},[url]);
...
export async function warmPdfWorker(){const r=await fetch(workerUrl);if(!r.ok)throw new Error('PDF-Anwendung nicht verfügbar.');await r.arrayBuffer();}
